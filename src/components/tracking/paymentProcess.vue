...
import PaymentProcessData from '~/pages/tracking/paymentprocessdata.json';

// Fetching data from json file
const paymentProcessData = PaymentProcessData;

// V-Model Variables
const datas = ref<Tracking[]>(paymentProcessData as unknown as Tracking[]);
...
<template>
    <div class="w-full bg-accentwhite pb-3 px-5 drop-shadow-md rounded-lg dark:bg-primarydark">  
        <h3 class="text-lg text-left dark:text-accentwhite ml-4 mt-5 mb-5">Payment Process</h3>
        <!-- Parent Table -->
        <DataTable 
        :value="datas"
        stripedRows
        ...
        <Column field="paymentStatus" header="Payment Status" class="flex justify-center items-center">
            <template #body="slotProps">
                <p class="text-sm rounded-lg px-2 py-1 w-28 text-center" 
                :class="{'bg-green-500'
                : slotProps.data.paymentStatus.toLowerCase() === 'fully paid', 
                'bg-yellow-500':
                slotProps.data.paymentStatus.toLowerCase() === 'partial paid'}"
                >
