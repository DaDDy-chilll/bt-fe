...
import RefundData from '~/pages/tracking/refunddata.json';

// Fetching data from json file
const refundData = RefundData;

// V-Model Variables
const datas = ref<Tracking[]>(refundData as unknown as Tracking[]);
...
<template>
    <div class="w-full bg-accentwhite pb-3 px-5 drop-shadow-md rounded-lg dark:bg-primarydark">  
        <h3 class="text-lg text-left dark:text-accentwhite ml-4 mt-5 mb-5">Refund</h3>
        <!-- Parent Table -->
        <DataTable 
        :value="datas"
        stripedRows
        ...
        <Column field="id" header="Order ID">
            <template #body="slotProps">
                <p class="text-sm text-accent1 dark:text-accent2">{{ slotProps.data.orderId }}</p>
            </template>
        </Column>
        <Column field="customerName" header="Customer Name" />
        <Column field="phoneNumber" header="Phone Number" />
        <Column field="email" header="Email" />
        <Column field="refundQuantity" header="Refund Quantity" class="text-center"/>
        <Column field="refundDate" header="Refund Date" />
